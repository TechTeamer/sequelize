  ...
  _run(connection, sql, parameters) {
    ...
    if (_.startsWith(self.sql, 'BEGIN TRANSACTION')) {
      ...
      if (Object.keys(self.outParameters).length > 0) {
        ...
        return connection.execute(self.sql, Object.assign(self.options.bind, self.outParameters), { outFormat: self.outFormat, autoCommit: self.autoCommit }).then(result => {
          if (benchmark) {
        ...
        return connection.execute(sqlToExec, self.options.bind || {}, { outFormat: self.outFormat, autoCommit: self.autoCommit }).then(result => {
          if (benchmark) {
            self.sequelize.log('Executed (' + (connection.uuid || 'default') + '): ' + self.sql, (Date.now() - queryBegin), self.options);
          }
