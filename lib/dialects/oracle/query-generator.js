      ...
      if (valueHash.hasOwnProperty(key)) {
        ...
        if (modelAttributeMap && modelAttributeMap[key] && modelAttributeMap[key].autoIncrement === true && !value) {
          ...
          if (currAttribute && currAttribute.type != null && (currAttribute.type.key === DataTypes.TEXT.key || currAttribute.type.key === DataTypes.BLOB.key)) {
            ...
            values.push(':field_' + i);
            if (!options.bind) {
              options.bind = {};
            }
            options.bind['field_' + i] = value;

            if (modelAttributeMap[key].type.constructor.name === 'BOOLEAN') {
              options.bind['field_' + i] = modelAttributeMap[key].type._stringify(value);
            } else {
              options.bind['field_' + i] = value;
            }
          }
 ...
 */
  updateQuery(tableName, attrValueHash, where, options, attributes) {
    ...
    for (const key in attrValueHash) {
      ...
      values.push(this.quoteIdentifier(key) + '=' + ':field_' + i);
    }
