...
const QueryGenerator = {
  ...
  addLimitAndOffset(options, model) {
    ...
    if (options.limit || options.offset) {
      const pkIdentifier = model.primaryKeyField;

      const alreadyOrderedByPk = () => {
        if (!(options.order instanceof Array)) {
          return false;
        }

        return options.order.some(order =>
          order instanceof Array ? order[0] === pkIdentifier : order === pkIdentifier
        );
      };


      if (!alreadyOrderedByPk() && (!(options.order && options.group) && (!options.order || options.include && !orders.subQueryOrder.length))) {
        fragment += options.order && !isSubQuery ? ', ' : ' ORDER BY ';
        fragment += this.quoteTable(options.tableAs || model.name) + '.' + this.quoteIdentifier(pkIdentifier);
      }
